...
@Autonomous(name="KalmanDistTest", group="TestTeleop")
public class KalmanDistTest extends AUTO_PRIME {
    ...
    public void runOpMode() {
        ...
        double START_DIST = 5; //in away from object to start off
        ...
        while(!isStopRequested()){
            ...
            u = robot.sensors.getFrontDist();
            x = x_previous + u; //may not be necessary since IMu should save its prev readout and auto add? but we'll see
            p = p_previous + Q;
            K = p/(p + R);

            z = robot.drive.getPoseEstimate().getX() + START_DIST + Math.tan(robot.drive.getPoseEstimate().getHeading())*robot.drive.getPoseEstimate().getX(); //get "actual" heading value
            x = x + K * (z - x);
            p = (1 - K) * p;
